...
int main()
{
    ...
    while(fin >> ax)
    {
        ...
        if(positive)
        {
            //cout << "positive" << endl;
            if(ax > 0)
                if(line_over_zero(ax,ay,bx,by) xor line_over_zero(ax,ay,cx,cy))
                    ct++;
            if(bx > 0)
                if(line_over_zero(ax,ay,bx,by) xor line_over_zero(bx,by,cx,cy))
                    ct++;
            if(cx > 0)
                if(line_over_zero(cx,cy,bx,by) xor line_over_zero(ax,ay,cx,cy))
                    ct++;
        }
        else
        {
