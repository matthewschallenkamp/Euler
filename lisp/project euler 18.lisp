...
(defun collapse-row (upper lower)
  (let ((outrow))
    (loop
       (if (= 0 (length lower))
	  (return (reverse outrow)))
       (setf outrow (cons (+ (car lower) (max (first upper) (second upper))) outrow))
       (setf lower (cdr lower))
       (setf upper (cdr upper)))))

(defun collapse-triangle (triangle)
  (let ((rtriangle (reverse triangle)))
    (loop
       (if (= 1 (length rtriangle))
	   (return (car rtriangle)))
       (let ((upper (first rtriangle)) (lower (second rtriangle)))
	 (setf rtriangle (cdr rtriangle))
	 (setf (car rtriangle) (collapse-row upper lower))))))
