...
{
  if (a.num == "0") *this = b;
  else if (b.num == "0") *this = a;
  else
  {
    *this = a;
    string bs = b.num;

    if (bs.size() < num.size()) bs.resize(num.size(), '0');

    if (num.size() < bs.size()) num.resize(bs.size(), '0');
    bool carry = false;
...
{
  for (int i = b.num.size() - 1; i >= 0; i--) out << b.num[i];
  return out;
}
