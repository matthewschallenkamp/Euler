...
vector<long long>pentagonals_to_n(long long n)
{
  ...
  while (n > p.back()) p.push_back(p.size() * (3 * p.size() - 1) / 2);
  return p;
}
...
vector<long long>pentagonals(int n)
{
  ...
  if (n >= p.size())
  {
    for (int i = p.size(); i <= n; i++) p.push_back(i * (3 * i - 1) / 2);
  }
  return p;
}
...
int main()
{
  ...
  cout << "The minimum difference was " << dmin << "." << endl;
  return 0;
}
