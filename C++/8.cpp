...
// the largest multiple of 12 adjacent numbers
...
int main()
{
  ...
  {
    c = v[i] * v[i - 1] * v[i - 2] * v[i - 3] * v[i - 4] * v[i - 5] * v[i - 6] *
      v[i - 7] * v[i - 8] * v[i - 9] * v[i - 10] * v[i - 11] * v[i - 12];
    m = max(m, c);
  }
  cout << "The greatest product was " << m << "." << endl;
  return m;
}
